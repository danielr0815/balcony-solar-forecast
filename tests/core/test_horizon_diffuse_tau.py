...
Covers (ADR §3.8 unit part):
  - SVF golden: a wall sector (az 195..360, el 90, tau 0) with ``diffuse_tau 0.5``
    lifts the sky-view factor, matches an independent brute-force quadrature, and
    obeys the exact wall blend identity SVF(rho) == rho + (1 - rho) * SVF0 (the
    M4 mechanism: the live diagnostic 0.288 -> ~0.64 follows the same identity);
  - ``diffuse_tau=None`` is BIT-IDENTICAL to the plain opaque wall (pre-0.22);
...
def _wall_plane(diffuse_tau=None, *, az=205.0, tilt=70.0):
    rows = (
        ...
        HorizonRow(_WALL_HI, 90.0, 0.0, diffuse_tau=diffuse_tau),
    )
    return PlaneConfig(
...
def test_diffuse_tau_wall_blend_identity():
    ...
    # The ADR live figure (0.288 -> ~0.64 at rho 0.5) is this same identity.
    base = H.sky_view_factor(_wall_plane(None))
...
def test_diffuse_tau_none_bit_identical_to_opaque_wall():
    # None must reproduce the pre-0.22 opaque-wall SVF bit-for-bit.
    assert H.sky_view_factor(_wall_plane(None)) == H.sky_view_factor(
        _wall_plane(None)
    )
    # And the plain wall (no field at all) equals a wall built without the arg.
    plain = PlaneConfig(
        name="M4", azimuth_deg=205.0, tilt_deg=70.0, wp=430.0,
        horizon=(
            HorizonRow(0.0, 0.0, 1.0),
            HorizonRow(_WALL_LO, 90.0, 0.0),
            HorizonRow(_WALL_HI, 90.0, 0.0),
        ),
    )
    assert H.sky_view_factor(_wall_plane(None)) == H.sky_view_factor(plain)
