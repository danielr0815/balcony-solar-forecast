...
MISSING_SITE_MESSAGE = (
    ...
    "    Settings -> Devices & Services -> Balcony Solar Forecast -> the "
    "entry's\n"
    "    three-dot menu -> 'Reconfigure' ('Neu konfigurieren'). Its 'site' "
    "object\n"
    "    selector holds the live object; copy it into site.json (the "
    "SiteConfig.from_dict\n"
    "    shape). NOT the 'Configure' button: that opens the options flow, which "
    "renders\n"
    "    the runtime tunables only and has no site field.\n"
    "    On the HA host the same object sits in .storage/core.config_entries as "
    "the\n"
    "    entry's data.site (structural config lives in entry.data; a stale "
    "options.site\n"
    "    only exists on entries never reconfigured since the reconfigure flow "
    "landed).\n"
    "    Then pass --site site.json.\n"
    "\n"
