...
in-process ``run_bootstrap`` HA action. This script is the DEV-machine CLI
wrapper: it owns the network layer (aiohttp: Open-Meteo Previous-Runs + HA
WebSocket LTS with a long-lived token), hands the fetched weather records +
per-module hourly actuals to the core, and writes the JSON. The pure core names
are re-exported below so ``backfill.<name>`` keeps addressing them (the
unit-test anchor in ``tests/core/test_backfill_math.py``).
